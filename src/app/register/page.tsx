...
import { AlertCircle, Mail, Lock, Shield, Eye, EyeOff, Home, ArrowRight, Loader2, User, Phone, Users, Briefcase, ArrowLeft } from 'lucide-react';

type UserType = 'household' | 'helper' | null;
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!userType) {
      setError('Please select whether you are a household or a helper');
      return;
    }
    ...
    try {
      ...
      // Redirect to profile completion page based on user type
        ...
        <Card className="bg-white/5 border-white/10 backdrop-blur-sm shadow-2xl">
          <CardHeader className="text-center space-y-4">
            ...
            <CardDescription className="text-gray-300 text-lg">
              Create your account to get started with trusted domestic help
            </CardDescription>
          </CardHeader>
            ...
            {!userType && (
              <div className="space-y-6">
                <div className="text-center">
                  <p className="text-white font-medium text-lg mb-6">I am a:</p>
                </div>
                <div className="grid grid-cols-1 gap-4">
                  <Button
                    onClick={() => setUserType('household')}
                    variant="outline"
                    className="h-auto p-6 border-white/20 text-white hover:bg-primary/20 hover:border-primary/50 transition-all duration-300 group"
                  >
                    <div className="flex items-center space-x-4 w-full">
                      <div className="w-12 h-12 bg-primary/20 text-primary rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="h-auto p-6 border-white/20 text-white hover:bg-secondary/20 hover:border-secondary/50 transition-all duration-300 group"
                  >
                    <div className="flex items-center space-x-4 w-full">
                      <div className="w-12 h-12 bg-secondary/20 text-secondary rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                        <Briefcase className="h-6 w-6" />
                      </div>
                      <div className="text-left flex-1">
                        <h3 className="font-semibold text-lg text-white group-hover:text-secondary transition-colors duration-300">
                          Helper
                        </h3>
                        <p className="text-sm text-gray-300">Offering my domestic services</p>
                      </div>
                      <ArrowRight className="h-5 w-5 text-gray-400 group-hover:text-secondary group-hover:translate-x-1 transition-all duration-300" />
                    </div>
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center space-x-3 pb-4 border-b border-white/20">
                  ...
                  <div className="flex items-center space-x-2">
                    <Badge variant="outline" className={`border-${userType === 'household' ? 'primary' : 'secondary'}/30 text-${userType === 'household' ? 'primary' : 'secondary'} bg-${userType === 'household' ? 'primary' : 'secondary'}/10`}>
                      {userType === 'household' ? <Users className="h-3 w-3 mr-1" /> : <Briefcase className="h-3 w-3 mr-1" />}
                      {userType === 'household' ? 'Household' : 'Helper'}
                    </Badge>
                  </div>
                </div>
